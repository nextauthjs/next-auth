...
const NavBar: Component<INavBarProps> = () => {
  ...
  return (
    // we have to call session to trigger suspense, this doesn't really do anything
    <Show when={!session() || session()}>
      <header class="flex flex-col w-full gap-2 fixed left-2/4 right-2/4 -translate-x-2/4 items-center">
        <nav class="w-2/4  lg:w-[40%]  p-5 bg-[#0000000d] flex items-center justify-between rounded-lg">
          <Show
            when={session()?.user}
            keyed
            fallback={
