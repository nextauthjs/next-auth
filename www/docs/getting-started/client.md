  ...
  return (
    <Provider
      ...
      options={{
        ...
        keepAlive: 5 * 60,
        // Don't send session broadcast to other tabs
        disableSessionEventBroadcast: false,
        // Don't send logout broadcast to other tabs
        disableLogoutEventBroadcast: false,
      }}
    >
      <Component {...pageProps} />
    </Provider>
...
Every tab/window maintains its own copy of the local session state; the session is not stored in shared storage like localStorage or sessionStorage. Any update in one tab/window triggers a message to other tabs/windows to update their own session state, unless you set `disableSessionEventBroadcast` and/or `disableLogoutEventBroadcast`.
...
`disableSessionEventBroadcast` is useful in a situation where you are dealing with refresh tokens and multiple tabs trying to check the session and refresh the token at the sime time will result in errors.
