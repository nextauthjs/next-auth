...
import NextAuth, { CustomCredentials } from "next-auth"
import Credentials from "next-auth/providers/credentials"

class InvalidLoginError extends CustomCredentials {
...
You will then receive that custom error code in the query parameters of the signin page your user returns to after a failed login attempt, for example `https://app.company.com/auth/signin?error=CustomCredentials&code=Invalid+identifier+or+password`.
