...
The Passkeys provider requires an additional table called `Authenticator`. Below is a migration in raw SQL
...
If you're using Auth.js in Next.js via middleware, you'll need to make some small adjustments as Prisma does not support edge runtimes yet. See the [Prisma docs](/getting-started/adapters/prisma#edge-compatibility) for more details.
...
The Passkeys provider has some unique options, for more details, check out the [API reference](/reference/providers/passkey). Below are some of the common options and their default values.
...
export const { handlers, auth } = NextAuth({
  providers: [
    Passkey({
      ...
      simpleWebAuthnBrowserVersion: 'v9.0.1',
      authenticationOptions: { timeout: 5 * 60 * 1000 },
      registrationOptions: { timeout: 5 * 60 * 1000 },
      formFields: {
        email: {
          label: "Email",
          required: true,
