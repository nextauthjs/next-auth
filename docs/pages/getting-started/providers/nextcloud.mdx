...
const response = await NextAuth({
  providers: [
    Nextcloud({ clientId: process.env.AUTH_NEXTCLOUD_ID, clientSecret: process.env.AUTH_NEXTCLOUD_SECRET, issuer: process.env.AUTH_NEXTCLOUD_ISSUER }),
  ],
})
```
...
import { AUTH_NEXTCLOUD_ID, AUTH_NEXTCLOUD_SECRET, AUTH_NEXTCLOUD_ISSUER } from "$env/static/private"
