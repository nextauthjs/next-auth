...
<img align="right" src="/img/providers/mailersend.svg" height="64" width="96" />

# Mailersend Provider

## Overview

The Mailersend provider uses email to send "magic links" that contain URLs with verification tokens can be used to sign in.

Adding support for signing in via email in addition to one or more OAuth services provides a way for users to sign in if they lose access to their OAuth account (e.g. if it is locked or deleted).

The Mailersend provider can be used in conjunction with (or instead of) one or more OAuth providers.
...
<Callout type="warning">
  The Mailersend provider can be used with both JSON Web Token and database
...
1. First, you'll need to [add your domain](https://app.mailersend.com/domains) to your Mailersend account. This is required by Mailersend and this domain of the address you use in the `from` provider option.

2. Next, you will have to generate an API key with sending access in the [Mailersend Dashboard](https://app.mailersend.com/api-tokens). You can save this API key as the `AUTH_MAILERSEND_KEY` environment variable.

```sh
AUTH_MAILERSEND_KEY=abc
```

If you name your environment variable `AUTH_MAILERSEND_KEY`, the provider will pick it up automatically and your Auth.js configuration object can be simpler. If you'd like to rename it to something else, however, you'll have to manually pass it into the provider in your Auth.js configuration.
...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  providers: [
    Mailersend({
      ...
      apiKey: MAILERSEND_MAGIC_LINKS_TOKEN,
...
export const { onRequest, useSession, useSignIn, useSignOut } = QwikAuth$(
  () => ({
    providers: [
      Mailersend({
        // If your environment variable is named differently than default
        apiKey: import.meta.env.MAILERSEND_MAGIC_LINKS_TOKEN,
        from: "no-reply@company.com",
      }),
    ],
  })
...
export const { handle, signIn, signOut } = SvelteKitAuth({
  ...
  providers: [
    Mailersend({
      // If your environment variable is named differently than default
      apiKey: env.MAILERSEND_MAGIC_LINKS_TOKEN,
      from: "no-reply@company.com",
    }),
  ],
})
...
You can fully customize the sign in email that is sent by passing a custom function as the `sendVerificationRequest` option to `Mailersend()`.
...
As an example, the following shows the source for our built-in `sendVerificationRequest()` method. Notice that we're rendering the HTML (`html()`) and making the network call (`fetch()`) to Mailersend to actually do the sending here in this method.

```ts {4, 14} filename="./lib/authSendRequest.ts"
