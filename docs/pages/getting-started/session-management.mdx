...
Auth.js supports both server-side and client-side Signin and Signout methods.
...
```ts filename="app/client/page.tsx" {9}
"use client"
...
Be aware, this is an async function that needs to be awaited. That means you'll need to use it in an `async` component (i.e. a React Server Component) or in a `useEffect` hook, for example.
...
```tsx filename="./components/UserAvatar" {6}
"use client"
...
```svelte filename="src/components/user-avatar.svelte" {6}
<script lang="ts">
  import { page } from "$app/stores"
</script>
...
Protecting pages also comes in two forms: server-side and client-side.

For server-side protection, you can use the `auth` function returned from `NextAuth` and exported from your Auth.js configuration file directly. Client-side we can use the `useSession` hook from `next-auth/react`.

```tsx filename="app/client/page.tsx" {6}
"use client"
...
API Routes in SvelteKit work like any other server-side file in Auth.js in SvelteKit, you can access the session by calling `event.locals.auth()`.
