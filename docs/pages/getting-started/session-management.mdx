...
For signing in users, assuming you have [already setup an authentication method](/getting-started/authentication), you only need to build a button that when pressed calls the sign in function exported in your Auth.js config.

<Callout>
  Note that when logging in for the first time, Auth.js will automatically
  create an account for the user in the database.
</Callout>
