...
First, we can use the `SignIn` component exported from `@auth/sveltekit/components`.
...
And for that `+page.server.ts` to run, it also needs a `+page.svelte` file, which we can leave "empty" like this.
...
```ts filename="src/routes/signout/+page.svelte"
<!-- empty -->
```
...
In Auth.js we need to call the `auth()` helper which will return the session data.
...
With SvelteKit, you've already set the session on your `$page` store in your `+layout.server.ts` file, therefore we can use it directly like this.
