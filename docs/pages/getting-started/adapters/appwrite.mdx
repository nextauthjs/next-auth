...
<Callout>
This adapter is developed by the community and not officially maintained or supported by Appwrite. It uses the Appwrite Database to store user and session data in a separate `next_auth` database. It is a standalone Auth server that does not interface with Appwrite Auth and therefore provides a different feature set.
</Callout>
