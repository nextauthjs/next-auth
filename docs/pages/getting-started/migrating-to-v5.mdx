...
1. Create an `auth.config.ts` file with your config and export it. You can put all common configuration here that does not rely on the adapter.
...
3. In your middleware file, import the first `auth.config` configuration object and use it to [lazily initialize]() Auth.js there, without the non-edge compatible adapter.
