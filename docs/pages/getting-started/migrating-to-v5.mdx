...
<Callout type="warning">
  ...
  `next-auth/react` server-side, you'll have to refactor them to use
  [`auth()`](/reference/nextjs#auth) instead.
</Callout>
...
3. In your middleware file, import the configuration object from your first `auth.config.ts` file and use it to lazily initialize Auth.js there. In effect, initializing Auth.js separately with all of your common options, but **without the edge incompatible adapter**.
