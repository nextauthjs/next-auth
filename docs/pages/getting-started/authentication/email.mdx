  ...
  >
    <RichTabs.List className="flex gap-4 flex-wrap">
      ...
      >
        ...
        <div className="text-sm text-center">Loops.so</div>
      </RichTabs.Trigger>
...
This provider only passes one data varaiable into the template, `url` which is the magic link to sign in. This is case sensitive, so make sure you use `url` in your template on the last page of Template creation, you'll need to copy the `TRANSACTIONAL ID`. If you skipped this step, don't worry, you can get this at any time by clicking on your template and switching to the Publish tab.

### Create an API Key on Loops

You'll need to create an API key to authenticate with Loops.so. This key should be kept secret and not shared with anyone.
...
To implement Loops, you need to set up the following environment variables:
```bash filename=".env"
