...
Can't find yours? You can [configure any OAuth provider](/guides/configuring-oauth-providers#use-your-own-provider), even if it's not on the list.
...
<Callout type="info">
  Auth.js can persist sessions in a cookie. Using a database is therefore
  optional, unless you want to persist user data in your own database or a
...
Official database adapters are available under the `@auth/` scope on npm. Their source code is available in the [`nextauthjs/next-auth`
monorepo](https://github.com/nextauthjs/next-auth/tree/main/packages).
