...
<ListDisclosure limit={8}>
  {Object.entries(manifest.providersOAuth).map(([id, name]) => (
    <Link
      ...
      key={name}
      className="p-4 border border-solid dark:border-neutral-800 border-neutral-200 rounded-lg flex flex-col items-center justify-between w-36 shadow-sm h-32 dark:border-neutral-800 dark:hover:bg-neutral-950 hover:bg-neutral-50 transition-colors duration-300"
    >
