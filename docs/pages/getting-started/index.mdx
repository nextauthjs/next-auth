...
<ListDisclosure limit={8}>
  {Object.entries(manifest.providersOAuth).map(([id, name]) => (
    <Link
      ...
      key={name}
      className="flex h-32 w-36 flex-col items-center justify-between rounded-lg border border-solid border-neutral-200 p-4 !no-underline shadow-sm transition-colors duration-300 hover:bg-neutral-50 dark:border-neutral-800 dark:border-neutral-800 dark:hover:bg-neutral-950"
    >
