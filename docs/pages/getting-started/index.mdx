...
The only required environment variable is `AUTH_SECRET`. Generate one via the [Auth.js CLI](https://cli.authjs.dev):
...
<small>
- _Must_ be a cryptographically random value, as it is used to seal cookies and hash
  [email](#email) verification tokens
- The CLI updates the **.env** file too, respecting the framework's conventions
- Multiple values are supported for rotating secrets, without invalidating existing sessions. See [Rotating Secrets](/guides/rotating-secrets)
