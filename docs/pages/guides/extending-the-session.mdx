...
<Callout type="info">
  All callbacks are asnyc functions, so you can also get extra information from a database or external APIs.
</Callout>
...
If you need to persist session data server-side, you must save it elsewhere. You can connect to the database in the `session()` callback to retrieve this information.
</Callout>
