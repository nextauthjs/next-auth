...
<Code>
<Code.Next>
To get the data on the server side, you need to use the `auth` function and verify if the user has the expected role. The information exposed by the function is what is set through the `session` and `jwt` callbacks within the configuration file.
