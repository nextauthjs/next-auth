...
```svelte filename="src/routes/signin/+page.svelte" {13-17} /submitButton/
<script lang="ts">
...
You'll need to create this server action in SvelteKit to handle the action at `/signin`. That's the default path, but this can be changed with the `signInPage` prop on the `SignIn` component above.
