...
:::info
Auth.js is customizable - [our guides section](/guides/overview) will teach you how to set it up to handle auth in different ways. All the possible configuration options are [listed here](/reference/configuration/auth-config).
:::
...
Protecting your API Routes (blocking unauthorized access to resources) is easy! You can either: use [`getSession()`](/reference/utilities/#getsession) to know whether a session exists or not:
...
Next, you'll be presented with the following screen which presents all the configurations for your new OAuth app. For now, we need two things from it: the **Client ID** and **Client Secret** for our new OAuth app:
...
:::warning
🔥 Keep both your Client ID and Client Secret secure and never expose them to the public or shared them with people outside your organization. With them, a malicious actor could hijack your application and cause you and your user serious problems!
:::
