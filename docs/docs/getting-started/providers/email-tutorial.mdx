...
Aside from authenticating users in Auth.js via [OAuth](/getting-started/providers/oauth-tutorial), you can also enable the option to authenticate them via "magic links". These are links that are sent to the user's email and when clicking on them they'll sign up the user automatically.
...
:::tip
The Email Provider can be used with both JSON Web Tokens and database sessions, but you [must configure a database adapter](/getting-started/adapters) to use it. It is not possible to enable email sign in without using a database.
:::
