...
1. Add your config: [`src/providers/{provider}.ts`](https://github.com/nextauthjs/next-auth/tree/main/packages/next-auth/src/providers)<br />
    ...
    - Add two SVG's of the provider logo, like `google-dark.svg` and `google-light.svg`, to the `/packages/next-auth/provider-logos/` directory as well as the styling config to the provider config object. See existing provider for example
