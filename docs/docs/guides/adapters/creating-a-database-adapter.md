...
So for the most basic use case, you will need to implement the following methods:
...
The following methods are currently optional as they aren't invoked by Auth.js yet, but recommended:

- [`deleteUser`](/reference/core/adapters#deleteuser)
- [`unlinkAccount`](/reference/core/adapters#unlinkaccount)

### Database models

The most basic user management model only needs the following tables/columns:
...
To add verification token support, expand your database tables/columns as follows. Notice the `emailVerified` column in the `User` table and the `VerificationToken` table:
...
See [Verification Token](/getting-started/adapters#verification-token) models
...
1. [Adapter tests](https://github.com/nextauthjs/next-auth/tree/main/packages/adapter-test) _must_ be included and _must_ pass. Docker is favored over services, to make CI resilient to network errors and to reduce the number of GitHub Action Secrets (which also lets us run these tests in fork PRs)
