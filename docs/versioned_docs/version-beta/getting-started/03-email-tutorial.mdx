...
import smtpConfig from "./img/sendinblue-smtp.png"
...
:::info
For this tutorial, we're gonna be using [Sendinblue](https://www.sendinblue.com/), but any of the services linked above should work the same
:::

First create an account in and then login to see the dashboard:

<img src={smtpConfig} />

1. Navigate to `SMTP & API`
2. Copy **hots**,**port**,**login** and **password** to a safe location

Next, let's add the config we copied to a safe location as environment variables in our NextJS app:
...
SMTP_PROT=587
EMAIL_FROM=sith@darkside.com
```

Note that we're also specifying from which domain email are going to be sent from. You'll need to create a "Sender" in **Sendinblue** by linking the email account you want to send emails as to it. Navigate to "Sender & IPs" and follow the instructions.
