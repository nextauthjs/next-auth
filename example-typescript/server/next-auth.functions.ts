 ...
 **/
import { IFunctions, ISendSignInEmailOpts } from "../../";
...
export default () => {
  ...
  }).then((usersCollection: Collection) => {
    const options: IFunctions<{id?: string, _id?:string, name?:string, email?:string, emailVerified?:boolean, admin?:boolean}> = {
      // If a user is not found find() should return null (with no error).
      find: ({id, email, emailToken, provider} = {id: null, email: null, emailToken: null, provider: null}) => {
        let query = {};
      ...
      sendSignInEmail: ({ email, url }: ISendSignInEmailOpts):Promise<boolean> => {
        return nodemailer.createTransport(nodemailerTransport).sendMail(
            {
              to: email,
