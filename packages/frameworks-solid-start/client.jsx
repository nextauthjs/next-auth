...
const __NEXTAUTH = {
    ...
    baseUrlServer: parseUrl(process.env.AUTH_URL_INTERNAL ??
        process.env.AUTH_URL ??
        process.env.VERCEL_URL).origin,
    basePathServer: parseUrl(process.env.AUTH_URL_INTERNAL ?? process.env.AUTH_URL).path,
    _lastSync: 0,
    _session: undefined,
    _getSession: () => { },
};
...
export function useSession(options) {
    ...
    if (!value && import.meta.env.DEV) {
        throw new Error("[solid-auth]: `useSession` must be wrapped in a <SessionProvider />");
    }
    ...
    if (requiredAndNotLoading) {
        return createMemo(() => ({ data: value().data, status: "loading" }));
    }
    return value;
}
/**
