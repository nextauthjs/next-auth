import { createContext, useContext, createResource, onMount, onCleanup, createEffect, } from 'solid-js';
import { getRequestEvent, isServer } from 'solid-js/web';
import { conditionalEnv, getEnv, now } from './utils';
import { parseUrl } from './utils';
export const __SOLIDAUTH = {
...
export function createSession() {
    ...
    if (!value && import.meta.env.DEV) {
        throw new Error('[@solid-mediakit/auth]: `createSession` must be wrapped in a <SessionProvider />');
    }
    return value;
}
...
export function SessionProvider(props) {
    ...
    __SOLIDAUTH._session = event?.locals?.session;
    const [session, { refetch }] = createResource(async (_, opts) => {
    ...
    createEffect(() => {
        ...
        onCleanup(() => document.removeEventListener('visibilitychange', visibilityHandler, false));
    });
...
export async function signIn(providerId, options, authorizationParams) {
    ...
    const signInUrl = getUrl(`/api/auth/${isCredentials ? 'callback' : 'signin'}/${providerId}`);
...
export const getSession = async (event) => {
    ...
    if (isServer && event?.request && event?.response) {
