...
const signin = async (provider, args) => {
  ...
  if (!providers[provider]) {
    ...
    window.location.href = `${providers[provider].signinUrl}?callbackUrl=${encodeURIComponent(callbackUrl)}`
  } else {
...
// TODO: determine whether this is the correct typing of `formData`
const _encodedForm = (formData: Record<string, string>) => {
  return Object.keys(formData).map((key) => {
    return encodeURIComponent(key) + '=' + encodeURIComponent(formData[key])
  }).join('&')
}

// TODO: get stricter record type to JSON subset
const _sendMessage = (message: Record<string, any>) => {
