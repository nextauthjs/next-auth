...
const Adapter = (typeOrmConfig, options = {}) => {
  ...
  async function getAdapter(appOptions) {
    ...
    async function getUserByAttribute(attribute, value) {
      debugMessage("GET_USER_BY_ATTRIBUTE", attribute, value);
      try {
        if (!value || !attribute) {
          return Promise.resolve(null);
        }
        return connection.getRepository(User).findOne({ [attribute]: value });
      } catch (error) {
        logger.error("GET_USER_BY_ATTRIBUTE_ERROR", error);
        return Promise.reject(new Error("GET_USER_BY_ATTRIBUTE_ERROR", error));
      }
    }
