...
const postgres = (models: Models, options) => {
  ...
  if (!customModels.User) {
    const {columns} = models.User.schema;
    type ColumnKey = keyof typeof columns;
    const columnKeys = Object.keys(columns) as ColumnKey[];

    columnKeys.forEach((columnKey) => {
      if (models.User.schema.columns[columnKey].type === 'timestamp') {
        // TODO: figure out a way without this mutation
        // @ts-ignore
        models.User.schema.columns[columnKey].type = 'timestamptz'
      }
    })
  }

  if (!customModels.Account) {
    const {columns} = models.Account.schema;
    type ColumnKey = keyof typeof columns;
    const columnKeys = Object.keys(columns) as ColumnKey[];

    columnKeys.forEach((columnKey) => {
      if (models.Account.schema.columns[columnKey].type === 'timestamp') {
        // TODO: figure out a way without this mutation
        // @ts-ignore
        models.Account.schema.columns[columnKey].type = 'timestamptz'
      }
    })
  }

  if (!customModels.Session) {
    const {columns} = models.Session.schema;
    type ColumnKey = keyof typeof columns;
    const columnKeys = Object.keys(columns) as ColumnKey[];

    columnKeys.forEach((columnKey) => {
      if (models.Session.schema.columns[columnKey].type === 'timestamp') {
        // TODO: figure out a way without this mutation
        // @ts-ignore
        models.Session.schema.columns[columnKey].type = 'timestamptz'
      }
    })
  }

  if (!customModels.VerificationRequest) {
    const {columns} = models.VerificationRequest.schema;
    type ColumnKey = keyof typeof columns;
    const columnKeys = Object.keys(columns) as ColumnKey[];

    columnKeys.forEach((columnKey) => {
      if (models.VerificationRequest.schema.columns[columnKey].type === 'timestamp') {
        // TODO: figure out a way without this mutation
        // @ts-ignore
        models.VerificationRequest.schema.columns[columnKey].type = 'timestamptz'
      }
    })
  }
}

const mongodb = (models: Models, options) => {
...
export default (config, models: Models, options) => {
