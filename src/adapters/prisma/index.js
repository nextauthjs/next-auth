...
const Adapter = (config) => {
  ...
  return {
    async getAdapter (appOptions) {
      const logger = prefixLogger('PRISMA', appOptions.logger)

      if (appOptions && (!appOptions.session || !appOptions.session.maxAge)) {
        logger.debug('GET_ADAPTER', 'Session expiry not configured (defaulting to 30 days')
      }
      ...
      return {
        ...
        async linkAccount (userId, providerId, providerType, providerAccountId, refreshToken, accessToken, accessTokenExpires) {
          logger.debug('LINK_ACCOUNT', { userId, providerId, providerType, providerAccountId, refreshToken, accessToken, accessTokenExpires })
          try {
        ...
        async deleteVerificationRequest (identifier, token, secret, provider) {
          logger.debug('DELETE_VERIFICATION', { identifier, token })
          try {
            // Delete verification entry so it cannot be used again
            const hashedToken = createHash('sha256').update(`${token}${secret}`).digest('hex')
            await prisma[VerificationRequest].delete({ where: { token: hashedToken } })
