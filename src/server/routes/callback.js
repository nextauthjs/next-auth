...
export default async (req, res, options, done) => {
  ...
  const sessionToken = req.cookies
    ? req.cookies[cookies.sessionToken.name]
    : null;
  ...
  if (type === "oauth") {
    try {
      oAuthCallback(
        req,
        provider,
        async (error, profile, account, OAuthProfile) => {
          try {
            if (error) {
            ...
            if (useJwtSession) {
              ...
              cookieExpires.setTime(
                cookieExpires.getTime() + sessionMaxAge * 1000
              );
            ...
            if (error.name === "AccountNotLinkedError") {
              ...
              res
                ...
                .setHeader(
                  "Location",
                  `${baseUrl}/error?error=OAuthCreateAccount`
                );
            } else {
    ...
    if (!user) {
      res
        ...
        .setHeader(
          "Location",
          `${baseUrl}/error?error=CredentialsSignin&provider=${encodeURIComponent(
            provider.id
          )}`
        );
      res.end();
    ...
    try {
      if (!adapter) {
        logger.error("WEBHOOK_REQUIRES_ADAPTER_ERROR");
        res
          .status(302)
          .setHeader("Location", `${baseUrl}/error?error=Configuration`);
        res.end();
      ...
      const identifier = attribute + ":" + value;
      ...
      // If is an existing user return a user object (otherwise use placeholder)
      const profile = (await getUserByAttribute(attribute, value)) || {
        [attribute]: value,
      };
      const account = {
        id: provider.id,
      ...
      console.log("[callback.js] account", account);
      // Check if user is allowed to sign in
      const signinCallbackResponse = await callbacks.signin(
        profile,
        account,
