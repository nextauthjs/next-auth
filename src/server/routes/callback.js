  ...
  if (provider.type === 'oauth') {
    ...
    try {
      ...
      try {
        ...
        return res.redirect(`${baseUrl}${basePath}/error?error=openIdCallback`)
      }
      ...
      try {
        ...
        // Note that the callback URL is preserved, so the journey can still be resumed
        if (isNewUser && pages.newUser) {
          return res.redirect(pages.newUser)
        }
        ...
        if (error.name === 'AccountNotLinkedError') {
          ...
          return res.redirect(`${baseUrl}${basePath}/error?error=OpenIdAccountNotLinked`)
        } else if (error.name === 'CreateUserError') {
          return res.redirect(`${baseUrl}${basePath}/error?error=OpenIdCreateAccount`)
        } else {
