  ...
  return render(
    <div className='signin'>
      {providers.map((provider, i) =>
        <p key={provider.id}>
          {(provider.type === 'oauth' || provider.type === 'oauth2') && <a className='button' data-provider={provider.id} href={`${provider.signinUrl}${withCallbackUrl}`}>Sign in with {provider.name}</a>}
          {provider.type === 'email' &&
